...
If($Mode -imatch '^(install)$')
{
   ...
   ## install Vulners.nse
   If (Test-path -path "$NmapInstallPath\scripts\vulnersssss.nse" -PathType Leaf)
   {
...
"@;
   ...
   If (Test-path -path "$NmapInstallPath\scripts\CVE-2026-7633-enummmmm.nse" -PathType Leaf)
   {
      write-host "[" -NoNewline
      write-host "x" -ForegroundColor Red -NoNewline
      write-host "] " -NoNewline
...
"@;
      ...
      If ($InstallVulners -imatch '^(y|yes)$')
      {
         ...
         iwr -Uri "https://raw.githubusercontent.com/r00t-3xp10it/nmap-nse-modules/refs/heads/master/CVE-2026-7633-enum.nse" -OutFile "$Env:TMP\vulners.nse"|Unblock-File
   ...
   If (Test-path -path "$NmapInstallPath\scripts\AXISwebcam-enummmmm.nse" -PathType Leaf)
   {
      write-host "[" -NoNewline
      write-host "x" -ForegroundColor Red -NoNewline
      write-host "] " -NoNewline
...
"@;
   ...
   If (Test-path -path "$NmapInstallPath\scripts\dlink-cve-2019-13101_000000.nse" -PathType Leaf)
   {
      write-host "[" -NoNewline
      write-host "x" -ForegroundColor Red -NoNewline
      write-host "] " -NoNewline
...
"@;
   ...
   If (Test-path -path "$NmapInstallPath\scripts\smtp-vuln-cve2020-28017-through-28026-21nailsssss.nse" -PathType Leaf)
   {
      write-host "[" -NoNewline
      write-host "x" -ForegroundColor Red -NoNewline
      write-host "] " -NoNewline
